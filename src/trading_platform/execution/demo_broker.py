...
class DemoBroker:
    ...
    def __init__(self, adapter: IExchangeAdapter) -> None:
        ...
        # exchange_order_id → (client Order, qty already reported as fills)
        self._open: dict[str, tuple[Order, Decimal]] = {}
    ...
    def submit_order(self, order: Order) -> list[Fill]:
        ...
        self._open[exchange_order_id] = (order, Decimal("0"))
        logger.info(
    ...
    def poll_fills(self) -> list[tuple[Order, Fill]]:
        ...
        for exchange_order_id, (order, reported_qty) in list(self._open.items()):
            status = self._adapter.fetch_order(exchange_order_id, order.symbol)
            fill = self._fill_delta(order, exchange_order_id, reported_qty, status)
            if fill is not None:
                produced.append((order, fill))
                reported_qty = reported_qty + fill.filled_qty
                self._open[exchange_order_id] = (order, reported_qty)
    ...
    def has_pending_order(self, symbol: str) -> bool:
        ...
        return any(order.symbol == symbol for order, _reported in self._open.values())

    def _fill_delta(
        self,
        order: Order,
        exchange_order_id: str,
        already_reported: Decimal,
        status: ExchangeOrderStatus,
    ) -> Fill | None:
        new_qty = status.filled_quantity - already_reported
        if new_qty <= 0:
            return None
        ...
        # Fee on the status is cumulative; attribute the incremental slice
        # proportionally when the venue only reports totals.
        fee = Decimal("0")
