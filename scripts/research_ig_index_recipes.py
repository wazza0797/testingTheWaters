#!/usr/bin/env python3
"""IG index CFD research: regime-gated recipes on 1h / 4h (default: FTSE).

Same low-frequency RegimeRouter playbooks as GBP/EUR research, pointed at
`config/ig-ftse.yaml` (Yahoo ^FTSE stand-in). Index spreads are tighter in
bps than FX, so the same recipes may survive costs better — still validate
any green result on full history + rolling OOS before promoting.

Overnight funding (interest + dividend adjustments on index CFDs) is
**not** modelled yet.

Usage:
    uv run python scripts/research_ig_index_recipes.py
    uv run python scripts/research_ig_index_recipes.py --timeframes 4h
    uv run python scripts/research_ig_index_recipes.py --overlay ig-ftse --recipes trend_ema
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--overlay",
        default="ig-ftse",
        help="Config overlay (default: ig-ftse).",
    )
    parser.add_argument(
        "--no-discord",
        action="store_true",
    ...
    settings = Settings()
    base_config = load_config(overlay=args.overlay)
    ...
    if rules is None:
        raise SystemExit(
            ...
            f"`uv run trading-platform download-data --overlay {args.overlay} --days 1` "
    ...
    for timeframe in timeframes:
        ...
        if not bars:
            print(
                f"{timeframe:<4} (no bars — run: uv run python scripts/backfill_ig_external.py --yahoo '^FTSE' --epic {symbol} --timeframe {timeframe})"
            )
            continue
    ...
    if rows:
        ...
        summary = "\n".join(
            [
                f"Research done: ig_index_recipes overlay={args.overlay}",
